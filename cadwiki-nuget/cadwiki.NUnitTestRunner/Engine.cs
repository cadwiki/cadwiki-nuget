...
namespace cadwiki.NUnitTestRunner
{
    ...
    public class Engine
    {
        ...
        {
            foreach (Tuple<Type, MethodInfo> item in tuples)
            {
                ...
                try
                {
                    suiteResult.AddMessage(Constants.vbLf + "Running test method: " + mi.Name);
                    object o;
        ...
        private static void AddFullExceptionToTestResult(TestResult testResult, TargetInvocationException ex)
        {
            ...
            List<string> stackTrace = Exceptions.GetStackTraceLines(ex);
