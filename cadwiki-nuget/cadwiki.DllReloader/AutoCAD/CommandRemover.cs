    ...
    public class CommandRemover
    {
        ...
        public static void RemoveAllCommandsFromiExtensionAppAssembly(Document doc, Assembly iExtensionAppAssembly, string dllPath)
        {
            ...
            if (doc is not null)
            {
                ...
                {
                    doc.Editor.WriteMessage(Constants.vbLf + "No commands found to remove.");
                }
                else
                {
                    doc.Editor.WriteMessage(Constants.vbLf + "Removing all commands from current assembly.");
        ...
        private static void RemoveCommands(Document doc, string dllPath, Dictionary<CommandMethodAttribute, MethodInfo> commandMethodAttributesToMethodInfos)
        {
            foreach (KeyValuePair<CommandMethodAttribute, MethodInfo> dictionaryItem in commandMethodAttributesToMethodInfos)
            {
                ...
                if (File.Exists(dllFolder + @"\" + "cadwiki.AcRemoveCmdGroup.dll"))
                {
                    ...
                    {
                        doc.Editor.WriteMessage(Constants.vbLf + "Command successfully removed: " + commandGlobalName.ToString());
                    }
                    else
                    {
                        doc.Editor.WriteMessage(Constants.vbLf + "Remove command failed: " + commandLineMethodString + ".");
                    }
                }
                else
                {
